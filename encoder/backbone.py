"""
backbone.py — Shared PNA + GATv2 encoder backbone.

All systems (NEURAL_META_K4, GNN_MOS_Classic, HyperOPT, NEURAL_ELA_MOS,
NEURAL_META_K2, ENSEMBLE_K4) import this single class instead of maintaining
divergent copies of _encode().

Key improvements over the original NeuralMetaheuristic._encode():
  - h_global re-injected between GATv2 layers (not just once before)
  - Returns 4-tuple: (h, e, h_per_head, h_global)
  - h_global is available downstream for cat([h, h_global]) in decision heads
  - All utility methods (load_pna, override_histogram, freeze/unfreeze) in one place
"""
import contextlib
import logging
...
class PNAGATv2Backbone(nn.Module):
    """Shared encoder: PNA (perception) + bridges + GATv2 (action).

    Returns 4-tuple from encode():
        h:          (N, gatv2_hidden)        per-node GATv2 output (all heads concatenated)
        e:          (E, gatv2_hidden)        edge embeddings
        h_per_head: (N, n_heads, head_dim)   raw attention head outputs (not mixed)
        h_global:   (B, pna_out_dim)         PNA graph-level readout

    h_per_head exposes the raw GATv2 attention heads. Each variante decides
    how to use them: reshape to K subpopulations, project, mix, or ignore.
    h_global is re-injected as a residual between GATv2 layers to prevent
    dilution of landscape-level information through message passing.
    """
    ...
    def __init__(self, node_in=NODE_DIM, edge_in=EDGE_DIM, global_in=GLOBAL_DIM,
                 ...
                 dropout=0.1, pna_checkpoint=None, device='cpu'):
        super().__init__()
        ...
        # ---- Bridges: PNA outputs → GATv2 inputs ----
        pna_cat_dim = pna_out * 2  # cat([h_nodes, h_node_edge])
        self.node_bridge = nn.Linear(pna_cat_dim, gatv2_hidden)
        self.edge_bridge = nn.Linear(pna_out, gatv2_hidden)
        self.global_bridge = nn.Linear(pna_out, gatv2_hidden)
        ...
        trainable = sum(p.numel() for p in self.parameters()
                        if p.requires_grad)
        log.info("PNAGATv2Backbone: %d total (%d PNA, %d trainable, n_heads=%d)",
    ...
    def encode(self, node_feat, edge_index, edge_attr, global_feat,
        ...
        """Run PNA + bridges + GATv2 with h_global re-injection.
        ...
        Returns:
            h:          (N, gatv2_hidden) per-node GATv2 output (concat of all heads)
            e:          (E, gatv2_hidden) edge embeddings
        ...
        # GATv2 with h_global re-injection between layers
    ...
    def encode_layerwise(self, node_feat, edge_index, edge_attr, global_feat,
        ...
        Returns dict with 8 representations (all on input device):
            ...
            h_post_bridge: (N, gatv2_hidden)  — after bridge + global inject
            h_gat0:        (N, gatv2_hidden)  — after GATv2 layer 0 + re-inject
            h_gatv2:       (N, gatv2_hidden)  — final (layer 1 + edge readout)
            h_global:      (B, pna_out)       — PNA graph-level readout
        ...
        # GATv2 layer 0
        h, e = self.gatv2_layers[0](h, edge_index, e)
        if len(self.gatv2_layers) > 1:
            if v_indices is not None:
                h = h + h_g[v_indices]
            else:
                h = h + h_g[0]
        ...
        # GATv2 layer 1 (if exists) + edge readout
    ...
    def override_degree_histogram(self, deg_hist=None):
        ...
        If deg_hist is None, uses the default calibrated histogram.
        """
        if deg_hist is None:
    ...
    def load_pna_checkpoint(self, checkpoint_path):
        ...
        self._expand_node_proj(state, 'node_proj.0.weight')
    ...
    def load_backbone_checkpoint(self, checkpoint_path):
        """Load full backbone weights from SSL next-step checkpoint.

        Handles node_proj width expansion (e.g. 8 → 9) for transfer
        between systems with different NODE_DIM.
        """
        ckpt = torch.load(checkpoint_path, map_location=self.device,
                          weights_only=False)
        ...
        self._expand_node_proj(state, 'pna.node_proj.0.weight')

        result = self.load_state_dict(state, strict=False)
        log.info("Loaded backbone from %s (missing=%s, unexpected=%s)",
                 checkpoint_path, result.missing_keys, result.unexpected_keys)
        self.override_degree_histogram()

    def _expand_node_proj(self, state, key):
        """Zero-pad node_proj weight if checkpoint has fewer input features."""
        if (key in state
                and state[key].shape[1] !=
            ...
            log.info("node_proj expanded: %d -> %d", old_in, new_w.shape[1])
    ...
    def get_param_groups(self, lr_pna=3e-5, lr_bridges=3e-4, lr_gatv2=3e-4):
        ...
        # GATv2 + edge_readout
        gatv2_params = (list(p for l in self.gatv2_layers for p in l.parameters()) +
                        list(self.edge_readout.parameters()))
        groups.append({'params': gatv2_params, 'lr': lr_gatv2, 'name': 'gatv2'})
        return groups
